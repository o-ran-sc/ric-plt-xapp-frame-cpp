...
.. Do NOT make changes directly to .rst or files. 
...
The xApp may also choose to use it's own send/receive loop, 
...
the object's constructor with two required parameters: 
      ...
      messages to be processed in parallel. By supplying a value 
      ...
      origin xApp. The xApp may still need to change the message 
      type and/or the subscription ID in the message prior to using 
      the reply function. 
      ...
      Figure 7: Send message without buffer a. 
      ...
      will need to send one or more messages before it can expect 
      ...
      response, all it needs to do is to register it's own callback 
      ...
      The RMR dump xApp isn't a traditional xApp inasmuch as it's 
      ...
      are in the examples directory of the C++ repo (link?). 
      ...
      how an application can control it's own listen loop while 
      sending messages. As with the other examples, some error 
      checking is skipped, and short cuts have been in order to 
      keep the example small and to the point. 
