...
std::shared_ptr<xapp::Jhash> xapp::Config::jparse( ) {
	char*	data;
	char	fname[MAX_PFNAME];			// max path is 4K, max filename is 255; should be the sum

	if( (data = getenv( (char *) "XAPP_DESCRIPTOR_PATH" )) == NULL ) {
		data =  (char *) ".";
	}
	snprintf( fname, sizeof( fname ), "%s/%s",  data, (char *) "config-file.json" );

	return jparse( std::string( fname ) );
}
